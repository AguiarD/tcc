...
<!-- </html> -->
  <script>
    function habilitar(){
      if(document.getElementById('idRepete').checked){
        document.getElementById('qtRepete').removeAttribute("disabled");
      } else {
        // document.getElementById('onoff').value
        document.getElementById('qtRepete').setAttribute("disabled", "disabled");
      }
    }
  </script>
  ...
  <body>
    ...
    <main>
      ...
      <div>
        <form method="POST" action="lancamentoIncluir.php">
          <div class="form-group">
            ...
            <input type="checkbox" id="idRepete" name="idRepete" onchange="habilitar()">
            <input type="number" id="qtRepete" name="qtRepete" placeholder="Quantas vezes?" value="1" disabled><br/><br/>
            
            <input type="submit" name="insert" class="btnVoltar" value="Inserir"><br/>
          </div>
          <nav>
            <a class="muda btn btn-info btnVoltar" href="lancamentos.php" onclick="">Voltar</a>
          </nav>
        </form>
      </div>
      <?php
        if (isset($_POST['insert'])) {
          ...
          for ($i=0; $i < $repetir; $i++) { 
            ...
            $dataPrevisao = strtotime($dataPrevisao);
            $dataPrevisao = strtotime('+1 month', $dataPrevisao);
            $dataPrevisao = date('Y/m/d', $dataPrevisao);
            // echo $dataPrevisao;
          }
        }
      ?>
